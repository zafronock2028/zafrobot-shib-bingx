...
KUCOIN_SECRET_KEY = os.getenv("SECRET_KEY")
KUCOIN_PASSPHRASE = os.getenv("API_PASSPHRASE")
...
# Funciones principales del trading
...
async def ejecutar_compra(par, precio, monto):
    try:
        ...
        orden = kucoin_client.create_market_order(
            symbol=par,
            side='buy',
            size=str(cantidad)
        )
...
async def monitorear_operacion(op):
    while op in operaciones and bot_activo:
        try:
            ticker = kucoin_client.get_ticker(symbol=op['par'])
